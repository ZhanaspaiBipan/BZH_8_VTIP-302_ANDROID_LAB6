...
public class MySQLite extends SQLiteOpenHelper {
    private static final int DATABASE_VERSION = 20; // НОМЕР ВЕРСИИ БАЗЫ ДАННЫХ И ТАБЛИЦ !

    static final String DATABASE_NAME = "headphones"; // Имя базы данных

    static final String TABLE_NAME = "headphone"; // Имя таблицы
    ...
    static final String WEBSITE = "websites";
    static final String WIRED = "wired";
    static final String SIZE = "size";
    static final String PAY = "pay";

    static final String ASSETS_FILE_NAME = "headphone.txt"; // Имя файла из ресурсов с данными для БД
    ...
    public void addData(SQLiteDatabase db, String name, String site,
                        String wired, String size, String payment) {
        ContentValues values = new ContentValues();
        values.put(NAME, name);
        values.put(NAME_LC, name.toLowerCase());
        values.put(WEBSITE, site);
        values.put(WIRED, wired);
        ...
        try {
            ...
            while ((str = in.readLine()) != null) { // Читаем строку из файла
                ...
                if (!strTrim.equals("")) { // Если строка не пустая, то
                    ...
                    String wired = st.nextToken().trim();
                    String size = st.nextToken().trim();
                    String payment = st.nextToken().trim();
                    addData(db, name, site, wired, size, payment); // Добавляем название и телефон в базу данных
                }
            }
    ...
    public String getData(String filter, Spinner spinner) {
        ...
        if (filter.contains("'")){
            ...
            selectQuery = "SELECT  * FROM " + TABLE_NAME + " WHERE ("
                    ...
                    + " OR " + WEBSITE + " LIKE '%" + filter.toLowerCase() + "%'"
                    + " OR " + WIRED + " LIKE '%" + filter + "%'"
                    + " OR " + SIZE + " LIKE '%" + filter + "%'"
                    + " OR " + PAY + " LIKE '%" + filter + "%'" + ")";
        } else if (idSpin == 1) {
            selectQuery = "SELECT  * FROM " + TABLE_NAME + " WHERE ("
            ...
            selectQuery = "SELECT  * FROM " + TABLE_NAME + " WHERE ("
                    + WEBSITE + " LIKE '%" + filter.toLowerCase() + "%'" + ")";
        } else if (idSpin == 3) {
            selectQuery = "SELECT  * FROM " + TABLE_NAME + " WHERE ("
                    + WIRED + " LIKE '%" + filter + "%'" + ")";
        } else if (idSpin == 4) {
        ...
        if (cursor.moveToFirst()) { // Если есть хоть одна запись, то
            do { // Цикл по всем записям результата запроса
                ...
                int p = cursor.getColumnIndex(WIRED);
                int l = cursor.getColumnIndex(SIZE);
                ...
                String wired = cursor.getString(p);
                String size = cursor.getString(l);
                String payment = cursor.getString(a);
                data.append(String.valueOf(++num) + ") Наушники:" + name +
                        "\n Сайт: " + site +
                        "\n Проводное: " + wired +
                        "\n Размер Наушников: " + size +
                        "\n Стоимость (тнг): " + payment + "\n");
